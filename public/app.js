...
function gangFights() {
  return state.settings?.fights || [];
}

function gangFactions() {
  return state.settings?.gangFactions || ["MG13"];
}

function fightStats() {
  const fights = gangFights();
  const wins = fights.filter((fight) => fight.result === "Gewonnen").length;
  const losses = fights.filter((fight) => fight.result === "Verloren").length;
  const noFights = fights.filter((fight) => fight.result === "No Fight").length;
...
function fightResultClass(result) {
  if (result === "Gewonnen") return "win";
  if (result === "Verloren") return "loss";
  return "nofight";
}
...
function renderTopbar() {
  $("#pageTitle").textContent = navLabel(state.page);
  $("#rankLine").textContent = state.page === "Dashboard" ? "MG13 Control Center" : pageDescription(state.page);
  $("#headerIcon").innerHTML = state.page === "Dashboard" ? "" : iconSvg(state.page);
  $("#headerIcon").classList.toggle("hidden", state.page === "Dashboard");
  $("#headerTitleBlock").classList.toggle("with-icon", state.page !== "Dashboard");
...
function renderDashboard() {
  ...
  content.innerHTML = `
    ...
    <section class="panel">
      <div class="panel-header">
        <h3>Notizen</h3>
        ${canAccess("actions", "manageNotes", "Supervisor") ? `<button class="blue-btn" id="addNoteBtn"><span>+</span> Notiz hinzufuegen</button>` : ""}
      </div>
      <div class="note-list">${state.notes.length ? state.notes.map(renderNote).join("") : `<p class="muted">Noch keine Notizen vorhanden.</p>`}</div>
    </section>
  `;
  $("#addNoteBtn")?.addEventListener("click", () => openNoteModal());
}
...
function renderFight() {
  ...
  content.innerHTML = `
    ...
    <section class="panel">
      <div class="panel-header"><h3>Fight Archiv</h3><button class="blue-btn" id="createFightBtn">${iconSvg("Plus")} Fight erstellen</button></div>
      <div class="fight-archive">
        ${gangFights().map(renderFightArchiveRow).join("") || `<p class="muted">Noch keine Fights eingetragen.</p>`}
      </div>
    </section>
  `;
  $("#createFightBtn")?.addEventListener("click", () => openFightModal());
  document.querySelectorAll(".edit-fight").forEach((button) => button.addEventListener("click", () => openFightModal(gangFights().find((fight) => fight.id === button.dataset.id))));
  document.querySelectorAll(".delete-fight").forEach((button) => button.addEventListener("click", () => openConfirmModal({
    title: "Fight loeschen",
    text: "Diesen Fight wirklich loeschen?",
    confirmText: "Fight loeschen",
    onConfirm: async () => {
      const data = await api(`/api/fights/${button.dataset.id}`, { method: "DELETE" });
      state.settings = data.settings;
      renderFight();
    }
  })));
...
function openFightModal(fight = null) {
  const isEdit = Boolean(fight);
  const types = ["5vs5", "10vs10", "15vs15", "Drop", "Staatsbank", "Juwe", "Dealer"];
  const results = ["Gewonnen", "Verloren", "No Fight"];
  openModal(`
    <h3>${isEdit ? "Fight bearbeiten" : "Fight erstellen"}</h3>
    <div class="form-grid">
      <label>Fight Art<select id="fightType">${types.map((type) => `<option ${fight?.type === type ? "selected" : ""}>${type}</option>`).join("")}</select></label>
      <label>Gegner<input id="fightOpponent" list="fightFactionList" value="${escapeHtml(fight?.opponent || "")}" placeholder="Fraktion eintragen"><datalist id="fightFactionList">${gangFactions().map((name) => `<option value="${escapeHtml(name)}"></option>`).join("")}</datalist></label>
      <label>Datum und Uhrzeit<input id="fightDateTime" type="datetime-local" value="${escapeHtml(toDatetimeLocal(fight?.dateTime))}"></label>
      <label>Ergebnis<select id="fightResult">${results.map((result) => `<option ${fight?.result === result ? "selected" : ""}>${result}</option>`).join("")}</select></label>
    </div>
    <p id="modalError" class="form-error"></p>
    <div class="modal-actions"><button class="ghost-btn" data-close>Abbrechen</button><button class="blue-btn" id="saveFight">${isEdit ? "Speichern" : "Fight erstellen"}</button></div>
  `, (modal) => {
    modal.querySelector("#saveFight").addEventListener("click", async () => {
      try {
        const body = JSON.stringify({ type: $("#fightType").value, opponent: $("#fightOpponent").value, dateTime: $("#fightDateTime").value, result: $("#fightResult").value });
        const data = await api(isEdit ? `/api/fights/${fight.id}` : "/api/fights", { method: isEdit ? "PATCH" : "POST", body });
        state.settings = data.settings;
        closeModal();
        renderFight();
      } catch (error) {
...
function renderGangIT() {
  content.innerHTML = `
    <section class="panel">
      <div class="panel-header"><h3>Fraktionen</h3><button class="blue-btn" id="addFactionBtn">${iconSvg("Plus")} Fraktion anlegen</button></div>
      <div class="edit-list">${gangFactions().map((name) => `
        <div class="edit-row faction-row"><span class="edit-name">${escapeHtml(name)}</span><button class="mini-icon danger delete-faction" data-name="${escapeHtml(name)}">${actionIcon("delete")}</button></div>
