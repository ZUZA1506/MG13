...
function normalizeGangFactions(value) {
  const list = Array.isArray(value) ? value : ["MG13"];
  return [...new Set(list.map((item) => String(item || "").trim()).filter(Boolean))];
}

function normalizeFights(value) {
  const validTypes = new Set(["5vs5", "10vs10", "15vs15", "Drop", "Staatsbank", "Juwe", "Dealer"]);
  const validResults = new Set(["Gewonnen", "Verloren", "No Fight"]);
  return (Array.isArray(value) ? value : [])
    .map((fight) => ({
      id: String(fight.id || makeId("fight")),
      type: validTypes.has(fight.type) ? fight.type : "5vs5",
      opponent: String(fight.opponent || "").trim(),
      dateTime: String(fight.dateTime || "").trim(),
      result: validResults.has(fight.result) ? fight.result : "No Fight",
      createdAt: String(fight.createdAt || nowIso()),
      updatedAt: String(fight.updatedAt || "")
    }))
    .filter((fight) => fight.opponent && fight.dateTime)
...
app.post("/api/fights", (req, res) => {
  ...
  if (!["5vs5", "10vs10", "15vs15", "Drop", "Staatsbank", "Juwe", "Dealer"].includes(type)) return res.status(400).json({ error: "Ungueltige Fight Art." });
  if (!["Gewonnen", "Verloren", "No Fight"].includes(result)) return res.status(400).json({ error: "Ungueltiges Ergebnis." });
  if (!opponent || !dateTime) return res.status(400).json({ error: "Gegner, Datum und Uhrzeit sind Pflichtfelder." });
  const fight = { id: makeId("fight"), type, opponent, dateTime, result, createdAt: nowIso(), updatedAt: "" };
  db.settings.fights = normalizeFights([fight, ...(db.settings.fights || [])]);
  if (!db.settings.gangFactions?.includes(opponent)) db.settings.gangFactions = normalizeGangFactions([...(db.settings.gangFactions || []), opponent]);
  writeDb(db);
...
app.patch("/api/fights/:id", (req, res) => {
  ...
  const patch = {
    ...fight,
    type: String(req.body.type || fight.type).trim(),
    opponent: String(req.body.opponent || fight.opponent).trim(),
    dateTime: String(req.body.dateTime || fight.dateTime).trim(),
    result: String(req.body.result || fight.result).trim(),
    updatedAt: nowIso()
  };
  db.settings.fights = normalizeFights(fights.map((item) => item.id === fight.id ? patch : item));
  if (!db.settings.gangFactions?.includes(patch.opponent)) db.settings.gangFactions = normalizeGangFactions([...(db.settings.gangFactions || []), patch.opponent]);
  writeDb(db);
